...
| `harbor import enex <file>` / `harbor export enex` | Evernote ENEX interchange. The import uploads straight to storage in chunks (any size), then waits for the import to finish — `--no-wait` returns as soon as it is queued and asks for a completion email by itself, since it is
the case where the email is your only signal — `--notify-email=true|false` overrides either way. `harbor import abort <job-id>` cancels an upload that never finished. |
